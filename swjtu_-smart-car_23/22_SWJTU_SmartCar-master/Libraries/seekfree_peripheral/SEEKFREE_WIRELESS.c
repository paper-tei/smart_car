...
uint8 wireless_rx_buffer,wireless_recieve_num;
uint8 wireless_recieve_buffer[10],num[10];

uint8 which_pid = 0;//1 p 2 i 3 d
//-------------------------------------------------------------------------------------------------------------------
...
void wireless_uart_callback(void)
{
    ...
    //读取收到的所有数据
	if(uart_query(WIRELESS_UART, &wireless_rx_buffer))
    { 
        if(wireless_recieve_num==0)
        {
            //判断命令对象   
            if(wireless_rx_buffer == 0x70)//对p操作
            {
                which_pid = 1;     
            }
            else if(wireless_rx_buffer == 0x69)//对i操作
            {
                which_pid = 2;
            }
            else if(wireless_rx_buffer == 0x64)//对d操作
            {
                which_pid = 3;
            } 
            else if(wireless_rx_buffer == 0x73)//对speed操作
            {
                which_pid = 4;
            }
            else if(wireless_rx_buffer == 0x6E)//对speed操作
            {
                which_pid = 5;
            }
        } 
        wireless_recieve_buffer[wireless_recieve_num++] = wireless_rx_buffer; 
        //跳出接收
        if(wireless_rx_buffer == 0x0A)
        {
            
            //处理数据
            for(uint8 a=2;a<=wireless_recieve_num-2;a++)
            {
                num[a-2] = wireless_recieve_buffer[a]-48;
            }
           // num[wireless_recieve_num-1] = '\0';
            switch (which_pid)
            {

            case 1:
                p = num[0]+0.1*num[2]+0.01*num[3];
                break;
            
            case 2:
                 i= num[0]+0.1*num[2]+0.01*num[3];
                break;

            case 3:
                d = num[0]+0.1*num[2]+0.01*num[3];
                break;
            case 4:
               // Speed_Left = 100*num[0]+10*num[1]+1*num[2];
            ...
            //清空变量
            wireless_recieve_num = 0;
            memset(wireless_recieve_buffer,0,sizeof(wireless_recieve_buffer));
